...
class LocateNode(Node):
    def __init__(self):
        ...
        self.is_goal_publisher = self.create_publisher(Bool, 'is_goal', 10)
        self.goal_info_publisher = self.create_publisher(
            Int32MultiArray, 'goal_info', 10)
    ...
    def image_callback(self, msg):
        ...
        # Your goal detection and position determination logic here
        is_goal, goal_info = self.detect_goal(cv_image)

        # Publish results to topics
        is_goal_msg = Bool()
        is_goal_msg.data = is_goal
        self.is_goal_publisher.publish(is_goal_msg)

        goal_info_msg = Int32MultiArray(data=goal_info)
        self.goal_info_publisher.publish(goal_info_msg)

    def detect_goal(self, cv_image):
        ...
        # Define color ranges for the goals in HSV format
        # These values are just placeholders; adjust them based on your goal colors
        lower_color_range = np.array([40, 100, 100])
        upper_color_range = np.array([80, 255, 255])
        ...
        is_goal = False
        goal_info = [0, 0, 0]  # Default coordinates and distance if the goal is not found

        if contours:
            # Assume the largest contour is the goal
            ...
            if shape:
                ...
                # Distance to the goal (simple conversion)
                distance_to_goal = int(np.sqrt(
                ...
                is_goal = True
                goal_info = [center_x_true, center_y_true, distance_to_goal]
                # goal_info = [int(robot_coordinates[0]), int(robot_coordinates[1]), int(robot_coordinates[2])]

        return is_goal, goal_info
